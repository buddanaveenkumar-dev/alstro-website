...
export default function CTAForm() {
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setErrors((prev) => ({ ...prev, [name]: validateField(name, value.trim()) }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch("https://formspree.io/f/xgvewqky", {
  ...
  const fieldClass = (name: string) => `input-field ${touched[name] && errors[name] ? "field-error" : ""}`;
  const selectClass = (name: string) => `${fieldClass(name)} cursor-pointer appearance-none bg-[url('data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2216%22 height=%2216%22 viewBox=%220 0 24 24%22 fill=%22none%22 stroke=%22%239CA3AF%22 stroke-width=%222%22><path d=%22M6 9l6 6 6-6%22/></svg>')] bg-no-repeat bg-[right_14px_center]`;

  return (
    <section id="contact" className="py-20 md:py-28 border-t border-border" aria-labelledby="contact-heading">
      <div className="container-main">
        <div className="reveal max-w-[480px] mb-10">
          <p className="section-label">Get in touch</p>
          <h2 id="contact-heading" className="mb-4">
            Request a shadow pilot or evaluation materials.
          </h2>
            ...
            <form ref={formRef} onSubmit={handleSubmit} noValidate className="space-y-5">
              ...
              <div>
                ...
                <select id="pilot-role" name="role" required defaultValue="" className={selectClass("role")} onBlur={handleBlur} aria-required="true" aria-invalid={!!errors.role} aria-describedby={errors.role ? "role-err" : undefined}>
                  ...
                  <option value="Fair Lending">Fair Lending Lead</option>
                  <option value="Model Risk">Model Risk / Governance</option>
              ...
              <p className="text-[12px] text-text-3 leading-relaxed">
                Your information will be used only to respond to this request and coordinate evaluation materials.
              </p>
            </form>
          )}
        </div>
