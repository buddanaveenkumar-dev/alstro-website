...
export default function HeroProofFeed() {
  ...
  return (
    <div className="glass-panel rounded-lg overflow-hidden font-mono text-[13px] leading-[1.8] relative shadow-2xl">
      <div className="absolute top-0 inset-x-0 h-[1px] bg-gradient-to-r from-transparent via-accent/50 to-transparent" />
      <div className="flex items-center justify-between px-5 py-3 border-b border-border/50 bg-black/20">
        <span className="text-text-3 text-[11px] uppercase tracking-[0.12em] font-medium">Decision intercepted</span>
      ...
      <div className="px-5 py-5 space-y-1 min-h-[340px] bg-surface/30">
        {feedLines.slice(0, visibleCount).map((line, i) => (
          <motion.div
            ...
            className={`flex items-center justify-between ${line.dim ? "text-text-3/30" : line.accent ? "text-accent font-semibold drop-shadow-[0_0_8px_rgba(59,130,246,0.3)]" : "text-text-2"}`}
          >
            <span className={line.accent ? "text-glow" : ""}>{line.text}</span>
            {line.check && (
              <span className={`ml-2 ${visibleCount > i ? "text-green drop-shadow-[0_0_4px_rgba(16,185,129,0.4)]" : "text-text-3"}`}>✓</span>
            )}
          </motion.div>
        ))}
      </div>
